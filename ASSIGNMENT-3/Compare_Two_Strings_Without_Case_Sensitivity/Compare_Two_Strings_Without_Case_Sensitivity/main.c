...
Que : 27. Write a C program which accept two strings from user and compare two strings without case sensitivity. If both strings are equal then return 0 otherwise return difference between first mismatch character.
			Eg:
...
void main() {
	...
	// Logic to compare two strings without case sensitivity.
	...
	while (arr1[i] != '\0') {
		...
		if (arr1[i] != arr2[i]) 
		{
			int high = arr1[i] > arr2[i] ? arr1[i] : arr2[i];
			int low = arr1[i] < arr2[i] ? arr1[i] : arr2[i];
			if (high - 32 == low) {
				i++;
				continue;
			}
