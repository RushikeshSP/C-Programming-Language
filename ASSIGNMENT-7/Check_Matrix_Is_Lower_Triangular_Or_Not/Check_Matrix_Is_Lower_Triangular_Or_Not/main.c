...
Que : 7. Write a C program to check whether given matrix is lower triangular or not (Using Dynamic Memory Allocation).
...
void main() {
	...
	// Logic to check given matrix is lower triangular or not..
	int flag = 0;
	for (int i = 1; i < row; i++) {
		for (int j = 0; j < i; j++) {
			if (*(*(p + j) + i) != 0) {
	...
	if (flag == 1) {
		printf("The given matrix is not an lower triangular matrix.");
	}
	else {
		printf("The given matrix is an lower triangular matrix.");
	}
}